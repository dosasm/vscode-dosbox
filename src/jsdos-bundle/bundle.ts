...
*/
export async function createBundle({ sample, boxConf, mount }: CreateBundleOptions): Promise<JSZip> {
    const zip = new JSZip();
    if (sample) {
    ...
    if (mount) {
        for (const m of mount) {
            await allFiles(m.dir, async (uri: vscode.Uri) => {
                ...
                const dst = "/home/web_user/" + m.disk + uri.path.replace(m.dir.path, "");
                zip.file(dst, arr);
            });
        }
    }
...
async function allFiles(dir: vscode.Uri, callback: (file: vscode.Uri) => Promise<void>) {
    const dirs = await fs.readDirectory(dir);
    const r: string[] | undefined = vscode.workspace.getConfiguration("vscode-dosbox").get("jsdos.ignore");
